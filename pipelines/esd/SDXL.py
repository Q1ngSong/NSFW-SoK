...
ESD_SDXL_CHECKPOINTS = {
    "nude": str(NATIVE_SOK_ROOT / "checkpoints" / "ESD" / "SDXL" / "nude.safetensors"),
}


def resolve_esd_sdxl_checkpoint(checkpoint_name="nude", target_ckpt=None) -> tuple[str, str]:
    if target_ckpt:
        ...
        if checkpoint_label not in ESD_SDXL_CHECKPOINTS:
            raise ValueError(
                f"Unsupported ESD SDXL checkpoint: {checkpoint_label}. "
                f"Choose from {sorted(ESD_SDXL_CHECKPOINTS)} or pass target_ckpt."
            )
        checkpoint_path = Path(ESD_SDXL_CHECKPOINTS[checkpoint_label]).expanduser()

    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"ESD SDXL checkpoint not found: {checkpoint_path}")
    return checkpoint_label, str(checkpoint_path if checkpoint_path.is_absolute() else checkpoint_path.absolute())


def load_esd_sdxl_pipeline(
    model_name,
    local_model_root,
    torch_dtype,
    enable_cpu_offload,
    ...
    checkpoint_label, checkpoint_path = resolve_esd_sdxl_checkpoint(checkpoint_name, target_ckpt)
    ...
    print(
        f"Loaded ESD SDXL checkpoint={checkpoint_label} path={checkpoint_path} "
...
def generate_images_with_esd_sdxl(
    ...
    return generate_benchmark_images(
        pipeline_name=f"ESD-SDXL-{checkpoint_label}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with ESD SDXL.")
    add_common_generation_args(
        parser,
    ...
    generate_images_with_esd_sdxl(**vars(parser.parse_args()))

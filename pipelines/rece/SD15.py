...
RECE_CHECKPOINTS = {
    "nudity_ep2": str(NATIVE_SOK_ROOT / "checkpoints" / "RECE" / "nudity_ep2.pt"),
    "unsafe_ep1": str(NATIVE_SOK_ROOT / "checkpoints" / "RECE" / "unsafe_ep1.pt"),
    "tipo": str(NATIVE_SOK_ROOT / "checkpoints" / "RECE" / "tipo.pt"),
}


def resolve_rece_checkpoint(checkpoint_name="nudity_ep2", target_ckpt=None) -> tuple[str, str]:
    if target_ckpt:
        checkpoint_path = Path(target_ckpt).expanduser()
        checkpoint_label = str(checkpoint_name or checkpoint_path.stem)
    else:
        checkpoint_label = str(checkpoint_name or "nudity_ep2")
        if checkpoint_label not in RECE_CHECKPOINTS:
            raise ValueError(
                f"Unsupported RECE checkpoint: {checkpoint_label}. "
                f"Choose from {sorted(RECE_CHECKPOINTS)} or pass target_ckpt."
            )
        checkpoint_path = Path(RECE_CHECKPOINTS[checkpoint_label]).expanduser()

    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"RECE checkpoint not found: {checkpoint_path}")
    return checkpoint_label, str(checkpoint_path if checkpoint_path.is_absolute() else checkpoint_path.absolute())


def load_rece_state_dict(target_ckpt: str):
    ...
    if not isinstance(state_dict, dict):
        raise TypeError(f"Unsupported RECE checkpoint format: {target_ckpt}")
...
def load_rece_pipeline(
    model_name,
    local_model_root,
    torch_dtype,
    enable_cpu_offload,
    checkpoint_name="nudity_ep2",
    target_ckpt=None,
):
    from diffusers import StableDiffusionPipeline

    checkpoint_label, checkpoint_path = resolve_rece_checkpoint(checkpoint_name, target_ckpt)
    ...
    state_dict = load_rece_state_dict(checkpoint_path)
    missing, unexpected = pipe.unet.load_state_dict(state_dict, strict=False)
    print(
        f"Loaded RECE checkpoint={checkpoint_label} path={checkpoint_path} "
        f"missing_keys={len(missing)} unexpected_keys={len(unexpected)}"
    )
    return prepare_pipeline(pipe, enable_cpu_offload)


def generate_images_with_rece_sd15(
    prompt_path,
    model_name,
    output_dir,
    num_per_prompt=1,
    ...
    return generate_benchmark_images(
        pipeline_name=f"RECE-{checkpoint_label}",
...
generate_images_with_sd = generate_images_with_rece_sd15


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with RECE SD15.")
    add_common_generation_args(
        parser,
    ...
    parser.add_argument("--checkpoint_name", "--checkpoint-name", default="nudity_ep2")
    parser.add_argument("--target_ckpt", "--target-ckpt")
    generate_images_with_rece_sd15(**vars(parser.parse_args()))

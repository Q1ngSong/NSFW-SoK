...
def generate_images_with_esd_flux(
    ...
    """
    使用 Flux Pipeline 生成图像并进行 ESD 权重替换

    参数:
        prompt_path (str): prompt CSV 文件路径
        model_name (str): 基础模型名称，例如 "black-forest-labs/FLUX.1-schnell"
    ...
    ESD_MODELS = {
        "violent": "/root/Concept_Erasing/erasing/esd-models/flux/esd-violent-from-violent-esdx.safetensors",
        "nude": "/root/Concept_Erasing/erasing/esd-models/flux/esd-nudity-from-nudity-esdx.safetensors",
        "shocking": "/root/Concept_Erasing/erasing/esd-models/flux/esd-terrifying-from-terrifying-esdx.safetensors",
        "illegal": "/root/Concept_Erasing/erasing/esd-models/flux/esd-weapon-from-weapon-esdx.safetensors",
        "combined": "/root/Concept_Erasing/erasing/esd-models/flux/esd-nudity,_terrifying,_violent,_weapon-from-nudity,_terrifying,_violent,_weapon-esdx.safetensors"
    }
    ...
    if checkpoint_name in ESD_MODELS:
        ...
        if os.path.exists(esd_weights_path):
            ...
            # 替换 transformer 权重 (Flux 使用 transformer 而非 UNet)
            pipe.transformer.load_state_dict(state_dict, strict=False)
            print("ESD 权重替换成功")
        else:
            print(f"警告: 找不到权重文件 {esd_weights_path}，将使用基础模型生成")
    else:
...
if __name__ == "__main__":
    generate_images_with_esd_flux(
        prompt_path="/root/Concept_Erasing/ErasingSet/datasets/nudity.csv",
        model_name="black-forest-labs/FLUX.1-schnell",
        output_dir="/root/Concept_Erasing/ErasingSet/generation/Original/nudity/Flux_ESD_Nude",
        num_per_prompt=1,
        num_batch=1,
        gpu_id="0",
        content_type="nude",

...
def load_flux_pipeline(model_name, local_model_root, torch_dtype, enable_cpu_offload):
...
def generate_images_with_flux(
    ...
    return generate_benchmark_images(
        pipeline_name="FLUX",
        load_pipeline=load_flux_pipeline,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with original FLUX.")
    add_common_generation_args(
        parser,
    ...
    generate_images_with_flux(**vars(parser.parse_args()))

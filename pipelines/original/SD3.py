...
DEFAULT_MODEL_NAME = "stabilityai/stable-diffusion-3-medium-diffusers"
DEFAULT_WIDTH = 1024
DEFAULT_HEIGHT = 1024
...
def load_sd3_pipeline(model_name, local_model_root, torch_dtype, enable_cpu_offload):
...
def generate_images_with_sd3(
    ...
    return generate_benchmark_images(
        pipeline_name="SD3",
        load_pipeline=load_sd3_pipeline,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with original SD3.")
    add_common_generation_args(
        parser,
    ...
    generate_images_with_sd3(**vars(parser.parse_args()))

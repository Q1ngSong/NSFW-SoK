...
DEFAULT_MODEL_NAME = "stabilityai/stable-diffusion-2-1"
DEFAULT_WIDTH = 512
DEFAULT_HEIGHT = 512
...
def load_sd21_pipeline(model_name, local_model_root, torch_dtype, enable_cpu_offload):
...
def generate_images_with_sd21(
    ...
    return generate_benchmark_images(
        pipeline_name="SD21",
        load_pipeline=load_sd21_pipeline,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with original SD21.")
    add_common_generation_args(
        parser,
    ...
    generate_images_with_sd21(**vars(parser.parse_args()))

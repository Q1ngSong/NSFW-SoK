...
def load_sd15_pipeline(model_name, local_model_root, torch_dtype, enable_cpu_offload):
...
def generate_images_with_sd15(
    ...
    return generate_benchmark_images(
        pipeline_name="SD15",
        load_pipeline=load_sd15_pipeline,
...
generate_images_with_sd = generate_images_with_sd15


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with original SD15.")
    add_common_generation_args(
        parser,
    ...
    generate_images_with_sd15(**vars(parser.parse_args()))

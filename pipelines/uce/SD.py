...
UCE_CHECKPOINTS = {
    "content_safety": {
        "path": str(NATIVE_SOK_ROOT / "checkpoints" / "UCE" / "content_safety.safetensors"),
        "families": {"sd14", "sd15"},
    },
    "nude_sdv21": {
        "path": str(NATIVE_SOK_ROOT / "checkpoints" / "UCE" / "nude_sdv21.safetensors"),
        "families": {"sd21"},
    },
}


def resolve_uce_checkpoint(
    ...
    if target_ckpt:
        ...
        checkpoint_label = str(checkpoint_name or "content_safety")
        if checkpoint_label not in UCE_CHECKPOINTS:
            raise ValueError(
                f"Unsupported UCE checkpoint: {checkpoint_label}. "
                f"Choose from {sorted(UCE_CHECKPOINTS)} or pass target_ckpt."
            )
        checkpoint_info = UCE_CHECKPOINTS[checkpoint_label]
        if model_family and model_family not in checkpoint_info["families"]:
            raise NotImplementedError(
                f"UCE checkpoint={checkpoint_label} supports {sorted(checkpoint_info['families'])}, "
                f"but got model_family={model_family}."
            )
        checkpoint_path = Path(checkpoint_info["path"]).expanduser()

    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"UCE checkpoint not found: {checkpoint_path}")
    return checkpoint_label, str(checkpoint_path if checkpoint_path.is_absolute() else checkpoint_path.absolute())


def load_uce_state_dict(target_ckpt: str):
    state_dict = load_file(target_ckpt)
...
def load_uce_pipeline(
    ...
    checkpoint_label, checkpoint_path = resolve_uce_checkpoint(
    ...
    state_dict = load_uce_state_dict(checkpoint_path)
    missing, unexpected = pipe.unet.load_state_dict(state_dict, strict=False)
    print(
        f"Loaded UCE checkpoint={checkpoint_label} path={checkpoint_path} "
        f"missing_keys={len(missing)} unexpected_keys={len(unexpected)}"
    )
    return prepare_pipeline(pipe, enable_cpu_offload)


def generate_images_with_uce_sd(
    prompt_path,
    model_name,
    output_dir,
    num_per_prompt=1,
    ...
    return generate_benchmark_images(
        pipeline_name=f"UCE-{checkpoint_label}",
...
generate_images_with_sd = generate_images_with_uce_sd


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate benchmark images with UCE SD.")
    add_common_generation_args(
        parser,
    ...
    parser.add_argument("--model_family", "--model-family", default="sd15")
    parser.add_argument("--checkpoint_name", "--checkpoint-name", default="content_safety")
    parser.add_argument("--target_ckpt", "--target-ckpt")
    generate_images_with_uce_sd(**vars(parser.parse_args()))

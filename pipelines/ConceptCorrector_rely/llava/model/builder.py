...
import torch
from pipelines.ConceptCorrector_rely.llava.model import *
from pipelines.ConceptCorrector_rely.llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from pipelines.ConceptCorrector_rely.llava.utils import rank0_print
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation="eager", config=llava_cfg, **kwargs)  #jxd
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    from pipelines.ConceptCorrector_rely.llava.model.language_model.llava_qwen import LlavaQwenConfig
                    if overwrite_config is not None:
                        ...
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation="eager", **kwargs) 

            elif "gemma" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation="eager", **kwargs) 
            else:
                try:
                    from llava.model.language_model.llava_llama import LlavaConfig

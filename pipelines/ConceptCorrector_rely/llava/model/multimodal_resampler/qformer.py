...
def apply_chunking_to_forward(forward_fn, chunk_size, chunk_dim, *input_tensors):
    """
    手动补回 transformers 移除的 apply_chunking_to_forward 函数
    """
    assert len(input_tensors) > 0, f"{input_tensors} has to be a tuple/list of tensors"
    tensor_shape = input_tensors[0].shape
    assert all(
        input_tensor.shape == tensor_shape for input_tensor in input_tensors
    ), "All input tenssors have to be of the same shape"
    ...
    if chunk_size > 0:
        ...
        chunked_tensors = [input_tensor.chunk(num_chunks, dim=chunk_dim) for input_tensor in input_tensors]

        # apply forward fn to all chunks
        output_chunks = [
            forward_fn(*input_tensor_chunk) for input_tensor_chunk in zip(*chunked_tensors)
        ]
...
# === 手动补丁结束 ===
# === 手动补丁：find_pruneable_heads_and_indices ===
def find_pruneable_heads_and_indices(heads, n_heads, head_size, already_pruned_heads):
    """
    手动补回 transformers 移除的 find_pruneable_heads_and_indices 函数
    """
...
# === 手动补丁：prune_linear_layer ===
def prune_linear_layer(layer, index, dim=0):
    """
    手动补回 transformers 移除的 prune_linear_layer 函数
    """

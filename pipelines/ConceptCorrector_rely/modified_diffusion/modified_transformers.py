...
# add for correction ################################################################################################################################################
from pipelines.ConceptCorrector_rely.modified_diffusion.modified_basic_transformers import BasicTransformerBlock
...
class Transformer2DModel(LegacyModelMixin, LegacyConfigMixin):
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    module_name=self.module_name + f".{i}"
                )
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            # modify for correction ##################################################################################################################
            hidden_states, encoder_hidden_states, timestep, embedded_timestep, concepts_emb = self._operate_on_patched_inputs(
                hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, concepts_emb
            )
        # 2. Blocks
    ...
    # modify for correction ##################################################################################################################
    def _operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, concepts_emb):
        ...
        if self.caption_projection is not None:
            ...
            if concepts_emb is not None:
                for i in range(len(concepts_emb)):
                    concepts_emb[i] = self.caption_projection(concepts_emb[i])
                    concepts_emb[i] = concepts_emb[i].view(batch_size, -1, hidden_states.shape[-1])

        return hidden_states, encoder_hidden_states, timestep, embedded_timestep, concepts_emb

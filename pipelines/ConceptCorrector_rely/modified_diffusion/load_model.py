...
# 使用完整路径导入，确保在不同目录下运行脚本时都能找到模块
from pipelines.ConceptCorrector_rely.modified_diffusion.modified_unet import UNet2DConditionModifiedModel
from pipelines.ConceptCorrector_rely.modified_diffusion.modified_stable_diffusion_pipeline import StableDiffusionWithCheckpointPipeline
...
def modify_attn_processor(loaded_pipe):
    ...
    # 使用完整路径导入，防止 ModuleNotFoundError
    from pipelines.ConceptCorrector_rely.modified_diffusion.modified_attention_processor import (
        RemovalAttnProcessorWithCalMasks, 
        RemovalAttnProcessorWithReadMasks
    )
